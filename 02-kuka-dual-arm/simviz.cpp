...
// simulation loop
void simulation(Sai2Model::Sai2Model* robot, Sai2Model::Sai2Model* robotObject, Simulation::Sai2Simulation* sim);
...
int main() {
	...
	runloop = true;
	thread sim_thread(simulation, robot, robotObject, sim);
    ...
    while (!glfwWindowShouldClose(window))
	{
		...
		graphics->updateGraphics(robotObject_name, robotObject);
...
void simulation(Sai2Model::Sai2Model* robot, Sai2Model::Sai2Model* robotObject, Simulation::Sai2Simulation* sim)
{
	...
	Eigen::VectorXd gravityTorques = Eigen::VectorXd::Zero(robot->dof());

	while (runloop) {
		// wait for next scheduled loop
		timer.waitForNextLoop();
		robot->gravityVector(gravityTorques);
